...
import { getActiveDisqualificationForCouple } from '@/lib/services/tournament-disqualifications'
...
type OrganizerAuthResult =
  | { error: NextResponse; userId: null }
  | { error: null; userId: string }

const authenticateOrganizer = async (tournamentId: string): Promise<OrganizerAuthResult> => {
  ...
  if (!permissionResult.hasPermission) {
    return {
      error: NextResponse.json(
        { success: false, error: permissionResult.reason || 'Permisos insuficientes' },
        { status: 403 }
      ),
      userId: null,
    }
...
const setCoupleEliminated = async (
  supabase: any,
  tournamentId: string,
  coupleId: string,
...
export async function POST(
  ...
  try {
    ...
    const { data: tournament, error: tournamentError } = await supabase
      .from('tournaments')
      .select('id, type, status, bracket_generated_at')
      .eq('id', tournamentId)
      .single()
    ...
    const activeDisqualification = await getActiveDisqualificationForCouple(tournamentId, coupleId, supabase)
    if (activeDisqualification) {
      return NextResponse.json(
    ...
    if (tournament.status === 'ZONE_PHASE') {
      ...
      if (pendingMatchesError) {
        throw new Error(`Error buscando partidos pendientes: ${pendingMatchesError.message}`)
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (disqualification.phase === 'ZONE_PHASE') {
      ...
      const canceledMatches = Array.isArray(disqualification.metadata?.canceled_matches)
        ? disqualification.metadata.canceled_matches as Array<{ id: string; previous_status?: string }>
        : []

      for (const match of canceledMatches) {
        if (!match.id || !match.previous_status) {
          continue
        }
      ...
      if (hierarchy?.parent_match_id) {
        ...
        if (hierarchy.parent_slot === 1) {
          parentUpdate.couple1_id = null
        } else {
          parentUpdate.couple2_id = null
        }
      ...
      const previousMatchState = disqualification.metadata?.previous_match_state as
        | { status?: string; winner_id?: string | null; result_couple1?: string | null; result_couple2?: string | null }
        | undefined

      const { error: matchUpdateError } = await supabase
        .from('matches')
        .update({
          status: previousMatchState?.status || 'PENDING',
          winner_id: previousMatchState?.winner_id || null,
          result_couple1: previousMatchState?.result_couple1 || null,
          result_couple2: previousMatchState?.result_couple2 || null,
        })
        .eq('id', match.id)
        .eq('tournament_id', tournamentId)
