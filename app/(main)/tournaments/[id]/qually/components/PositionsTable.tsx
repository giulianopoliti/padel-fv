...
const PositionsTable: React.FC<PositionsTableProps> = ({
  ...
  const fallbackStandings = React.useMemo<ZonePosition[]>(() => {
    return coupleInscriptions
      ...
      .map((inscription, index) => ({
        ...
        couples: {
          ...
          players_player1: inscription.couples!.players_player1
            ? {
                first_name: inscription.couples!.players_player1.first_name,
                last_name: inscription.couples!.players_player1.last_name,
              }
            : null,
          players_player2: inscription.couples!.players_player2
            ? {
                first_name: inscription.couples!.players_player2.first_name,
                last_name: inscription.couples!.players_player2.last_name,
              }
            : null,
        },
      }));
  ...
  const fetchZonePositions = React.useCallback(async () => {
    ...
    if (error) {
      ...
      setStandings((data && data.length > 0 ? data : fallbackStandings) as ZonePosition[]);
    }

    if (disqualificationsResponse?.ok) {
      const disqualificationsData = await disqualificationsResponse.json().catch(() => null);
      const zoneDisqualifications = (disqualificationsData?.disqualifications || [])
        .filter((disqualification: ActiveDisqualification) => disqualification.phase === 'ZONE_PHASE');
      setActiveDisqualifications(zoneDisqualifications);
    } else {
      setActiveDisqualifications([]);
    }
      
    setLoading(false);
  }, [fallbackStandings, tournament.id]);
  ...
  const handleConfirmDisqualification = async () => {
    ...
    try {
      const response = await fetch(`/api/tournaments/${tournament.id}/couples/${zonePosition.couple_id}/disqualify`, {
        ...
        body: shouldRevert
          ? undefined
          : JSON.stringify({ reason: 'Descalificacion administrativa desde Qually' }),
      });
      ...
      toast({
        ...
        description: shouldRevert
          ? 'La pareja vuelve a quedar habilitada para Qually.'
        ...
        <Table>
          ...
          <TableBody>
            {standings.map((zonePosition, index) => {
              ...
              const activeDisqualification = disqualificationsByCouple.get(zonePosition.couple_id);
              const isDisqualified = Boolean(activeDisqualification);
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <div className={pendingIsRevert ? 'bg-slate-50 border border-slate-200 rounded-lg p-3' : 'bg-red-50 border border-red-200 rounded-lg p-3'}>
