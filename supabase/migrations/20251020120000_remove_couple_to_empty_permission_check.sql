-- ============================================================================
-- Migration: Remove Permission Check from couple_to_empty_swap RPC
-- ============================================================================
--
-- CONTEXTO:
-- - swap_bracket_positions_atomic ya NO valida permisos (confiando en backend)
-- - couple_to_placeholder_swap NO valida permisos (confiando en backend)
-- - couple_to_empty_swap SI valida permisos (inconsistente y bloqueando ORGANIZADOR)
--
-- SOLUCIÓN:
-- Remover validación de permisos de couple_to_empty_swap para que:
-- 1. Sea consistente con las otras RPCs de bracket
-- 2. Confíe en backend que usa checkTournamentPermissions() correctamente
-- 3. Permita que ORGANIZADOR y ADMIN puedan usar la función
--
-- BACKEND API VALIDATION:
-- El endpoint /api/tournaments/[id]/enhanced-bracket-operations valida permisos
-- usando checkTournamentPermissions() que soporta:
-- - CLUB owners (ownership directo del club)
-- - ORGANIZADOR members (miembros de organización con acceso al club)
-- - ADMIN users (acceso total)
--
-- ============================================================================
...
BEGIN
  ...
  --     RETURN 'Insufficient permissions - user must belong to tournament club';
  --   END IF;
  --
  -- ============================================================================
...
COMMENT ON FUNCTION "public"."couple_to_empty_swap"("p_tournament_id" "uuid", "p_user_id" "uuid", "p_source_match_id" "uuid", "p_target_match_id" "uuid", "p_source_slot" "text", "p_target_slot" "text", "p_operation_id" "text") IS 'Updated to remove permission check - backend API handles permissions via checkTournamentPermissions(). Supports CLUB owners, ORGANIZADOR members, and ADMIN users.';
